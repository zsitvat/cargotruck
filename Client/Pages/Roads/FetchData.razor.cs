...
namespace Cargotruck.Client.Pages.Roads
{
    public partial class FetchData
    {
        ...
        protected async Task ShowPageAsync()
        {
            ...
            Roads = await client.GetFromJsonAsync<RoadsDto[]>($"api/roads/get?page={currentPage}&pageSize={pageSize}&sortOrder={sortOrder}&desc={desc}&searchString={searchString}&filter={filter}&dateFilterStartDate={dateFilter?.StartDate}&dateFilterEndDate={dateFilter?.EndDate}");
            StateHasChanged();
        }
        ...
        async Task Delete(int Id)
        {
            ...
            if (await js.InvokeAsync<bool>("confirm", $"{@localizer["DeleteAsync?"]} {r?.Purpose_of_the_trip} - {r?.Direction} ({r?.Id})"))
            {
