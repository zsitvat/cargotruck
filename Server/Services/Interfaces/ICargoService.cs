...
namespace Cargotruck.Server.Services.Interfaces
{
    ...
    {
        Task<List<CargoDto>> GetAsync(int page, int pageSize, string sortOrder, bool desc, string? searchString, string? filter, DateTime? dateFilterStartDate, DateTime? dateFilterEndDate);
        Task<List<CargoDto>> GetCargoesAsync();
        ...
        Task PostAsync(CargoDto data);
        Task PutAsync(CargoDto data);
